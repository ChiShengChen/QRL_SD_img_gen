...
# 輸出目錄
OUTPUT_DIR = Path("improved_comparison_results")
...
def load_cifar10_images(num_images_per_class: int = 16) -> Dict[str, List[torch.Tensor]]:
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Resize((64, 64)),  # 調整到與生成圖像相同尺寸
    ])
    ...
    for class_id, class_name in enumerate(CIFAR10_CLASSES):
        ...
        for img_idx in range(num_images_per_class):
            try:
                ...
                # 使用訓練好的actor進行rollout
                with torch.no_grad():
                    for step in range(50):  # 增加生成步數以改善品質
                ...
                if final_image is not None:
                    ...
                    # 保存圖像
                    torch.save(final_image, GENERATED_DIR / actor_type / class_name / f"gen_{img_idx}.pt")
...
def main():
    ...
    results_file = OUTPUT_DIR / "improved_comparison_results.json"
